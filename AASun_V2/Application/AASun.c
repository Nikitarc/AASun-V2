...
----------------------------------------------------------------------
	...
	24/05/16	ac	V1.19	Port to PVB V2.0
...
#define		AASUN_VERSION		((1u << 16) | 19u)
...
static	uint16_t				samples [(I_SENSOR_COUNT+2) * MAIN_SAMPLE_COUNT] ;	// +1 voltage, +1 phase corrected voltage
	...
	while (1)
	{
		...
		if (collectionCount == 0)
		{
			...
			if (collectionDataOk == 0)
			{
				...
				collectionData.adcBattV = adcValues [IX_BATTV] ;
				collectionDataOk = 1 ;
			}
			else
			{
				// collectionData is not free: drop data
			}
...
static	int32_t		vSqrtSum ;
static	int32_t		iSqrtSum [I_SENSOR_COUNT] ;
static	int32_t		powerSum [I_SENSOR_COUNT] ;
...
void	AASun ()
{
	...
	while (1)
	{
		...
		if (collectionDataOk == 1u)
		{
			...
			vSqrtSum = usqrt (acquiredData.voltSumSqr / COLLECTION_COUNT) ;
			computedData.vRms =  voltCal * vSqrtSum ;	// VOLT_SHIFT  bits left shifted
			...
			for (ii = 0 ; ii < I_SENSOR_COUNT ; ii++)
			{
				...
				int32_t			temp = usqrt (acquiredData.iData[ii].iSumSqr / (COLLECTION_COUNT / 64)) ;

				iSqrtSum [ii] = temp ;
				computedData.iData[ii].iRms      = pICfg->iCal     * temp ;					// I_SHIFT     bits left shifted

				temp = acquiredData.iData[ii].powerSum / (int32_t) COLLECTION_COUNT ;
				powerSum [ii] = temp ;
				computedData.iData[ii].powerReal = pICfg->powerCal * temp - pICfg->powerOffset ;	// POWER_SHIFT bits left shifted
			...
			// Set battery voltage in the variable
			aaSunVariable [ASV_BATTV] = (aaSunCfg.battvCal * collectionData.adcBattV + BATTV_SHIFT_ROUND) >> BATTV_SHIFT  ;
			...
			// Check ADC overflow
			if (acquiredData.vPeakAdcP > 500  ||  acquiredData.iData[0].iPeakAdcP > 500
#if (defined IX_I2)
					  ||  acquiredData.iData[1].iPeakAdcP > 500
#endif
#if (defined IX_I3)
			...
			if (displayWTest (DPYW_DISPLAY_WH))
			{
				...
				aaPrintf ("Inp %6d, Exp %6d, Div1 %6d, Div2 %6d",
						energyWh.energyImported, energyWh.energyExported, energyWh.energyDiverted1, energyWh.energyDiverted2) ;
#if (defined IX_I2)
				aaPrintf (", E2 %6d", energyWh.energy2) ;
#endif
#if (defined IX_I3)
		...
		if (0 == strcmp ("?", pCmd))
		{
			...
			aaPrintf ("var [n v]   Display / set variable\n") ;
			...
			aaPrintf ("ci  n v [a] Set iCal for sensor n (a => v=I*100)\n") ;
			aaPrintf ("cp  n v [p] Set powerCal for sensor n (p => v=P\n") ;
			aaPrintf ("cpo n v     Set power offset (v = W_offset*10)\n") ;
			...
			aaPrintf ("cbv [v]     Battery voltage (mv)\n") ;
...
*/
		...
		else if (0 == strcmp ("ds", pCmd) ||  0 == strcmp ("dss", pCmd))		// Display samples
		{
			...
			if (! statusWTest (STSW_NOT_SYNC))
			{
				...
				// Displays the ADC values + phase corrected voltage => I_SENSOR_COUNT+2 values per line
				...
				if (0 == strcmp ("dss", pCmd))
				{
					...
					nn = 20u ;
					pSample = (int16_t *) & samples [((MAIN_SAMPLE_COUNT / 2u) - 10u) * (I_SENSOR_COUNT + 2)] ;
				}
				for (ii = 0 ; ii < nn ; ii++)
				{
					for (uint32_t jj = 0 ; jj < I_SENSOR_COUNT + 2 ; jj++)
					{
						aaPrintf (" %4d", * pSample++) ;
					}
					aaPutChar ('\n') ;
		...
		else if (* pCmd ==  'c')
		{
			...
			if (0 == strcmp ("c?", pCmd))
			{
				...
				for (ii = 0 ; ii < I_SENSOR_COUNT ; ii++)
				{
					...
					aaPrintf ("cpo     %d %d\n", ii+1, (aaSunCfg.iSensor [ii].powerOffset * 10) >> POWER_SHIFT) ;
				}
			...
			else if (0 == strcmp ("cv", pCmd))		// Set voltCal
			{
				...
				{
					if (pArg2 != NULL  &&  * pArg2 == 'v')
					{
						voltCal = (arg1 << VOLT_SHIFT) / vSqrtSum ;
					}
					else
					{
						voltCal = arg1 ;
			...
			else if (0 == strcmp ("ci", pCmd))		// Set iCal
			{
				if (pArg1 != NULL  &&  arg1 > 0  &&  arg1 <= I_SENSOR_COUNT)
				{
					...
					if (pArg2 != NULL)
					{
						if (pArg3 != NULL)
						{
							aaSunCfg.iSensor [arg1].iCal = (((arg2 << I_SHIFT) + iSqrtSum [arg1] / 2) / iSqrtSum [arg1]) / 100 ;
						}
						else
						{
							aaSunCfg.iSensor [arg1].iCal = arg2 ;
						}
						aaPuts ("New ") ;
					}
					aaPrintf ("iCal I%d: %d\n", arg1+1, aaSunCfg.iSensor [arg1].iCal) ;
				}
			}

			else if (0 == strcmp ("cp", pCmd))		// Set pCal coef
			{
				if (pArg1 != NULL  &&  arg1 > 0  &&  arg1 <= I_SENSOR_COUNT)
				{
					arg1-- ;
					if (pArg2 != NULL  &&  arg2 != 0)
					{
						if (pArg3 != NULL)
						{
							aaSunCfg.iSensor [arg1].powerCal = ((arg2 << POWER_SHIFT) - aaSunCfg.iSensor [arg1].powerOffset + powerSum [arg1] / 2) / powerSum [arg1] ;
						}
						else
						{
							aaSunCfg.iSensor [arg1].powerCal = arg2 ;
						}
						aaPuts ("New ") ;
					}
					aaPrintf ("powerCal I%d: %d\n", arg1+1, aaSunCfg.iSensor [arg1].powerCal) ;
				}
			}

			else if (0 == strcmp ("cpo", pCmd))		// Set power offset
			{
				if (pArg1 != NULL  &&  arg1 > 0  &&  arg1 <= I_SENSOR_COUNT)
				{
					if (pArg2 != NULL)
					{
						aaSunCfg.iSensor [arg1-1].powerOffset = (arg2  << POWER_SHIFT) / 10 ;
						aaPuts ("New ") ;
					}
			...
			else if (0 == strcmp ("cpmax", pCmd))	// Set max diverted power
			{
				if (pArg1 != NULL &&  arg1 > 0   &&  arg1 < 3)	// arg1 = diverter index
				{
					...
					if (pArg2 != NULL &&  arg2 > 0)				// arg2 is diverter power
					{
						...
						{
							// Compute for current voltage
							arg3 = computedData.vRms >> VOLT_SHIFT ;
						}

						int32_t		sq = arg3 * arg3 ;
						int32_t		power = (230 * 230 * arg2 + (sq / 2)) / sq ;
						powerDiv_t 	* pDiv = & powerDiv [arg1] ;
						pDiv->powerDiverter230 = power ;
						pDiv->powerDiverterMax = power << POWER_DIVERTER_SHIFT ;
						pDiv->powerDiverterRref = ((230 * 230) << POWER_DIVRES_SHIFT) / power ;
					}
				}
				powerDiv_t * pDiv = & powerDiv [0] ;
				aaPrintf ("Div 1 power Max: %4d (230V)  Resistor %3d.%02d\n",
						  pDiv->powerDiverter230,
						  pDiv->powerDiverterRref >> POWER_DIVRES_SHIFT,
						  ((pDiv->powerDiverterRref & POWER_DIVRES_SHIFT_M) * 100) / POWER_DIVRES_SHIFT_V) ;
				pDiv++ ;
				aaPrintf ("Div 2 power Max: %4d (230V)  Resistor %3d.%02d\n",
						  pDiv->powerDiverter230,
						  pDiv->powerDiverterRref >> POWER_DIVRES_SHIFT,
						  ((pDiv->powerDiverterRref & POWER_DIVRES_SHIFT_M) * 100) / POWER_DIVRES_SHIFT_V) ;
			}
			...
			else if (0 == strcmp ("cbv", pCmd))
			{
				if (pArg1 != NULL  &&  arg1 > 0)
				{
					// Set battvCal
					aaSunCfg.battvCal = (arg1 << BATTV_SHIFT) / adcValues [IX_BATTV] ;
					aaPrintf ("Battery cal: %u\n", aaSunCfg.battvCal) ;
				}
				else
				{
					// Display battery voltage
					aaPrintf ("Battery mV: %u\n", aaSunVariable [ASV_BATTV]) ;
				}
			}
				...
				case 'd':
					{
						...
						for (ii = 1 ; ii < POWER_HISTO_MAX_WHEADER ; ii++)
						{
							aaPrintf (" %02d:%02d  %9d %9d %9d"
								...
								pPowerHistory->diverted2,
#if (defined IX_I2)
								pPowerHistory->power2,
#endif
#if (defined IX_I3)
