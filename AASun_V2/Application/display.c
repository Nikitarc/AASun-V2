...
{
	{	'C',	15,	0,	AA_GPIO_MODE_INPUT_DN },	// Input 1 : J8
	{	'B',	9,	0,	AA_GPIO_MODE_INPUT_DN },	// Input 2 : J9
	{	'B',	2,	0,	AA_GPIO_MODE_INPUT_UP },	// Input 3 / pulse counter input J10
	{	'B',	11,	0,	AA_GPIO_MODE_INPUT_UP },	// Input 4 / pulse counter input J11

	{	'D',	3,	0,	AA_GPIO_MODE_OUTPUT_PP },	// Output 3 : On board relay J15
	{	'B',	7,	0,	AA_GPIO_MODE_OUTPUT_PP },	// Output 4 : J14
} ;
...
static	void	pageDayEnergy1	(void) ;
static	void	pageDayEnergy2	(void) ;
static	void	pageTotEnergy1	(void) ;
static	void	pageTotEnergy2	(void) ;
...
static	void	pageDayEnergy1 (void)
{
	...
	pageLine (0, "Energy Day", "Wh", 2) ;

	len = aaSnPrintf (lineBuffer, sizeof (lineBuffer), "%u", dayEnergyWh.energyImported) ;
	pageLine (1, "EImp", lineBuffer, len) ;

	len = aaSnPrintf (lineBuffer, sizeof (lineBuffer), "%u", dayEnergyWh.energyExported) ;
	pageLine (2, "EExp", lineBuffer, len) ;

#if (defined IX_I2)
	len = aaSnPrintf (lineBuffer, sizeof (lineBuffer), "%u", dayEnergyWh.energy2) ;
...
static	void	pageDayEnergy2 (void)
{
	...
	pageLine (line++, "Energy Day", "Wh", 2) ;

#if (defined IX_I3)
	len = aaSnPrintf (lineBuffer, sizeof (lineBuffer), "%u", dayEnergyWh.energy3) ;
	pageLine (line++, "E3", lineBuffer, len) ;
#endif

	len = aaSnPrintf (lineBuffer, sizeof (lineBuffer), "%u", dayEnergyWh.energyDiverted1) ;
	pageLine (line++, "ED1", lineBuffer, len) ;

	len = aaSnPrintf (lineBuffer, sizeof (lineBuffer), "%u", dayEnergyWh.energyDiverted2) ;
	pageLine (line++, "ED2", lineBuffer, len) ;
}

//--------------------------------------------------------------------------------
// Display total energy counters
...
static	void	pageTotEnergy1 (void)
{
	...
	len = aaSnPrintf (lineBuffer, sizeof (lineBuffer), "%u", energyWh.energy2) ;
	pageLine (3, "E2", lineBuffer, len) ;
}
