...
#define	I_SENSOR_MAX			3		// Max count of managed current sensors
#define	I_SENSOR_COUNT			3		// Count of managed current sensors

#if (I_SENSOR_MAX > 3  ||  I_SENSOR_COUNT > I_SENSOR_MAX)
...
#define	ASV_BATTV				2		// Battery voltage
#define	ASV_USER1				3		// User counter 1
...
#define	BATTV_CAL				16483	// Default for 12V range
#define	BATTV_SHIFT				10
#define BATTV_SHIFT_ROUND		((int32_t) (1u << (BATTV_SHIFT - 1u)))
...
// The allowed configuration for modified PCB V1 and V2
#define	OPT_SELECTED		(OPT_LINKY | OPT_TEMPERATURE)
...
typedef struct
{
	...
	uint32_t		battvCal ;				// Battery voltage calibration
