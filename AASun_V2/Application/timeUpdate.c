...
aaPuts ("WIFI date not set\n") ;
		...
		{
			pTm->tm_mon++ ;		// In date from WIFI january is 0, we want 1
aaPrintf ("WIFI %u %u/%02u/%02u %02u:%02u:%02u\n", pTm->tm_wday, 1900+pTm->tm_year, pTm->tm_mon, pTm->tm_mday, pTm->tm_hour, pTm->tm_min, pTm->tm_sec) ;
...
//aaPuts ("timeNext reset\n") ;
	...
	{
		// Retry not allowed in IDLE state
		// This is the end of a manual date update
		timeStatus = STS_IDLE ;
		return ;
	}

	if (command == NEXT_START  &&  bNotFirstDate == false)
	{
		// Insert a delay before the 1st date update (WIFI not ready)
		bNotFirstDate = true ;
		timerStart  (TIMER_DATE_IX, 10) ;
		bEnd = true ;
